class_name StatBlock extends Node
...
const SCALAR_STATS: Dictionary = {
	StatID.ARMOUR: true,
	StatID.EVASION_RATING: true,
	StatID.ENERGY_SHIELD_MAX: true,
...
# --- Helpers
func create_modifier(stat: Variant, form: Variant, value: float) -> Dictionary:
	var stat_id: int = (stat if typeof(stat) == TYPE_INT else _convert_stat_name_to_id(str(stat)))
	
	if stat_id == -1:
		push_warning("StatBlock: Unknow stat '%s'" % stat)
		return {}
	...
	if form_id == -1:
		push_warning("StatBlock: Unknow form '%s'" % form)
		return {}
	...
	for modifier in modifiers:
		...
		if modifier is Array and modifier.size() == 3:
			...
			if not (modifier.has("stat") and modifier.has("form") and modifier.has("value")):
				push_warning("StatBlock: Bad modifier in '%s': %s" % [source_name, str(modifier)])
				continue
			mod = create_modifier(modifier.stat, modifier.form, modifier.value)
		else:
			push_warning("StatBlock: Bad modifier in '%s': %s" % [source_name, typeof(modifier)])
			continue
		if mod.is_empty():
			continue
		source.append(mod)
