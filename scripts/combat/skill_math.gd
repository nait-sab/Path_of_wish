...
static func build_packet(skill: SkillInstance, _get_stat: Callable, main_hand: Gear) -> DamagePacket:
	...
	if main_hand:
		...
		print("[SkillMath] Main Hand Weapon Physical : ", physical_range)
		print("[SkillMath] Main Hand Weapon Speed : %.2f", attack_speed)
	...
	if skill.final.get("uses_weapon", false):
		...
		if weapon_packet > 0.0 and physical_range != Vector2.ZERO:
			...
			print("[SkillMath] Physical Hit : %.2f", hit_physical)
			base_physical += hit_physical * weapon_packet
			
	print("[SkillMath] After Weapon -> Physical: %.2f, Fire: %.2f" % [base_physical, base_fire])
	...
	print("[SkillMath] After Support -> Physical: %.2f, Fire: %.2f" % [base_physical, base_fire])
	...
	print("[SkillMath] After Critical -> Physical: %.2f, Fire: %.2f" % [base_physical, base_fire])
